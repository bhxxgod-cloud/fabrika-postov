// ⚠️ КОПИЯ dressup.cjs + ОДИН ГАРД (07.08, приказ начальника «вход по паролю ЗАПРЕЩЁН»).
// Повод: сегодня первая же попытка входа паролём получила анти-бот блок. dressup при мёртвых куках
// молча уходит в smartLogin и ВВОДИТ ПАРОЛЬ — на этой партии так делать нельзя.
// Отличие от dressup.cjs РОВНО ОДНО: при NO_PASSWORD=1 smartLogin не входит вообще (сразу 'fail'),
// значит акк либо открывается по кукам, либо не трогается совсем.
// Скопировано, а не поправлено на месте, потому что dressup.cjs в этой папке лежит незакоммиченный
// другим чатом. ЛЕЧИТЬ: перенести флаг NO_PASSWORD в dressup.cjs, когда те правки будут в коммите,
// и удалить эту копию.
...
async function smartLogin(page, a, slug, log, snap) {
  // ГАРД: вход паролём запрещён приказом. Куки не сработали → выходим, акк не дёргаем.
  if (process.env.NO_PASSWORD === '1') { log('  ⛔ NO_PASSWORD=1: куки не открыли акк, а вход паролём запрещён — НЕ вхожу'); return 'nopw'; }
